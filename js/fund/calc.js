...
function calcPerformance(fund, startDate = null, endDate = null, benchmarkNavs = null) {
  ...
  // 5b. 累计回撤 ≥5% 的次数（独立回撤段数）
  let dd5Count = 0;
  {
    let currPeak = cumulativeNavs[0].cumNav;
    let maxDDPct = 0; // 当前回撤段的最大回撤幅度（正数）
    let inDD = false;
    cumulativeNavs.forEach(p => {
      if (p.cumNav >= currPeak) {
        // 创新高，回撤段结束
        if (inDD && maxDDPct >= 0.05) dd5Count++;
        currPeak = p.cumNav;
        maxDDPct = 0;
        inDD = false;
      } else {
        // 处于回撤中
        const ddPct = (currPeak - p.cumNav) / currPeak;
...
function generateNavFitAnalysis(selFundStats, benchFund) {
  ...
  if (fundCount === 1) {
    ...
    parts.push(`<div style="margin-bottom:8px;"><b style="color:var(--text);">2. 风险特征：</b>最大回撤 <span style="color:var(--green);font-weight:700;">${(maxDD * 100).toFixed(1)}%</span>，年化波动率 ${vol != null ? (vol * 100).toFixed(1) + '%' : '--'}，${riskComment}。${p.ddRecoveryDays != null ? `回撤修复耗时${p.ddRecoveryDays}天。` : ''}累计回撤≥5%共 <b>${p.dd5Count || 0}次</b>。</div>`);
    ...
    if (p.sortino != null && sharpe != null) {
      let sc = "";
      if (p.sortino > sharpe * 1.2) sc = "下行波动控制明显优于整体波动，尾部风险管理优秀";
      else if (p.sortino > sharpe) sc = "下行波动控制优于整体波动，尾部风险较小";
      else if (p.sortino < sharpe * 0.8) sc = "下行波动偏大，需关注极端行情下的应对";
      else sc = "下行波动与整体波动基本一致";
      parts.push(`<div style="margin-bottom:8px;"><b style="color:var(--text);">6. 下行风险（索提诺）：</b>索提诺比率 ${p.sortino.toFixed(2)}，${sc}。（夏普 ${sharpe.toFixed(2)}）</div>`);
    }
    ...
    // ② 多维评级
    const __rating = (val, sT, aT, bT, cT) => {
    ...
    const _rR = __rating(annRet, 0.20, 0.10, 0.03, 0);
    // 抗风险：回撤越小越好，反向评级
    const _rkR = (() => {
      const md = -maxDD;
      if (md == null) return "C";
    ...
    // ③ 持有建议（根据基金状态）
    let _hSug = "";
    const _fSt = s.f.status || "holding";
    if (_fSt === "holding") {
      if (sharpe != null && sharpe > 1 && annRet > 0.10 && maxDD > -0.15) _hSug = "建议继续持有，业绩优秀、风险可控。";
      else if (annRet < 0 || (sharpe != null && sharpe < 0)) _hSug = "建议审慎评估，当前亏损或风险补偿不足。";
      else _hSug = "可继续观察，暂不加仓。";
    } else if (_fSt === "tracking") {
      if (sharpe != null && sharpe > 1.5 && maxDD > -0.10) _hSug = "可择机建仓，性价比优秀、回撤可控。";
      else if (annRet < 0 || maxDD < -0.20) _hSug = "暂不建议入手，业绩或风险特征不佳。";
      else _hSug = "继续观望，等待更好入场时机。";
    } else if (_fSt === "exited") {
      _hSug = "该基金已退出，可关注未来是否重新具备投资价值。";
    }
    parts.push(`<div style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border);font-size:13px;">${_hSug}</div>`);
