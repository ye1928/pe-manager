...
function renderDashboard() {
  ...
  // 资产配置
  const fundAssets = holdingFunds.reduce((s, f) => s + (calcFund(f).totalCost || 0), 0);
  ...
  el.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:24px;">
      <div style="background:linear-gradient(135deg,var(--blue),#2563eb);border-radius:12px;padding:20px;color:white;">
        <div style="font-size:11px;opacity:0.85;margin-bottom:6px;">私募总投入</div>
        <div style="font-size:26px;font-weight:800;letter-spacing:-0.5px;">¥${fmtWan(totalInvested)}万</div>
        <div style="font-size:11px;opacity:0.7;margin-top:4px;">持仓 ${holdingFunds.length} 只</div>
      </div>
      <div style="background:linear-gradient(135deg,${netGain >= 0 ? 'var(--red)' : 'var(--green)'},${netGain >= 0 ? '#dc2626' : '#16a34a'});border-radius:12px;padding:20px;color:white;">
        <div style="font-size:11px;opacity:0.85;margin-bottom:6px;">综合收益</div>
        <div style="font-size:26px;font-weight:800;">${netGain >= 0 ? '+' : ''}${fmtWan(netGain)}万</div>
        <div style="font-size:11px;opacity:0.7;margin-top:4px;">浮动${fmtWan(totalFloating)} / 已实现${fmtWan(totalRealized)}</div>
      </div>
      <div style="background:var(--bg2);border-radius:12px;padding:20px;border:1px solid var(--border);">
        <div style="font-size:11px;color:var(--text3);margin-bottom:6px;">持仓市值</div>
        <div style="font-size:26px;font-weight:800;${pnlColor(totalFloating + totalInvested)}">¥${fmtWan(totalInvested + totalFloating)}万</div>
        <div style="font-size:11px;color:var(--text3);margin-top:4px;">业绩报酬 -${fmtWan(totalPerfFee)}</div>
      </div>
      <div style="background:var(--bg2);border-radius:12px;padding:20px;border:1px solid var(--border);">
        <div style="font-size:11px;color:var(--text3);margin-bottom:6px;">今日资讯</div>
        <div style="font-size:26px;font-weight:800;color:var(--blue);">${recentNews.length} 条</div>
        <div style="font-size:11px;color:var(--text3);margin-top:4px;">${newsList.length} 条已归档</div>
      </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:1.5fr 1fr;gap:16px;margin-bottom:24px;">
      ...
      <div style="background:var(--bg2);border-radius:12px;padding:16px;border:1px solid var(--border);">
        <div style="font-size:13px;font-weight:700;margin-bottom:12px;">📈 净值走势对比</div>
        <div style="height:220px;"><canvas id="dash-nav-chart"></canvas></div>
      </div>
      <!-- 资产配置 -->
      <div style="background:var(--bg2);border-radius:12px;padding:16px;border:1px solid var(--border);">
        <div style="font-size:13px;font-weight:700;margin-bottom:12px;">🥧 资产配置分布</div>
        <div style="height:220px;"><canvas id="dash-pie-chart"></canvas></div>
        <div style="display:flex;gap:12px;margin-top:8px;font-size:11px;color:var(--text3);">
          <span><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:var(--blue);margin-right:4px;vertical-align:middle;"></span>私募 ${totalAssets > 0 ? Math.round(fundAssets/totalAssets*100) : 0}%</span>
          <span><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:var(--red);margin-right:4px;vertical-align:middle;"></span>股票 ${totalAssets > 0 ? Math.round(stockAssets/totalAssets*100) : 0}%</span>
          <span><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:var(--yellow);margin-right:4px;vertical-align:middle;"></span>期货 ${totalAssets > 0 ? Math.round(futuresAssets/totalAssets*100) : 0}%</span>
        </div>
      </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:24px;">
      <!-- 近期资讯 -->
      <div style="background:var(--bg2);border-radius:12px;padding:16px;border:1px solid var(--border);">
        <div style="font-size:13px;font-weight:700;margin-bottom:12px;">📰 近期资讯</div>
        <div id="dash-news-list">
          ${recentNews.length === 0 ? '<div style="font-size:12px;color:var(--text3);padding:12px 0;">暂无资讯，请在「研报资讯」中录入</div>' : recentNews.map(a => `
            <div style="display:flex;align-items:flex-start;gap:8px;padding:8px 0;border-bottom:1px solid var(--border);font-size:12px;">
              <span style="flex-shrink:0;padding:1px 5px;border-radius:3px;font-size:9px;${a.sentiment === 1 ? 'background:rgba(239,68,68,0.12);color:var(--red);' : a.sentiment === -1 ? 'background:rgba(34,197,94,0.12);color:var(--green);' : 'background:var(--bg3);color:var(--text3);'}">${a.sentiment === 1 ? '正面' : a.sentiment === -1 ? '负面' : '中性'}</span>
              <div style="flex:1;min-width:0;">
                <div style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-weight:600;">${a.title || '无标题'}</div>
                <div style="font-size:10px;color:var(--text3);margin-top:2px;">${a.source || ''} · ${a.date || ''}</div>
              </div>
            </div>
          `).join('')}
        </div>
      ...
      <!-- 股票池快照 -->
      <div style="background:var(--bg2);border-radius:12px;padding:16px;border:1px solid var(--border);">
        <div style="font-size:13px;font-weight:700;margin-bottom:12px;">📈 股票池快照（持仓）</div>
        <div id="dash-stock-list">
          ${holdingStocks.length === 0 ? '<div style="font-size:12px;color:var(--text3);padding:12px 0;">暂无持仓股票，请在「股票」页面添加</div>' : holdingStocks.slice(0, 8).map(s => {
            const c = calcStock(s);
            return `<div style="display:flex;align-items:center;gap:8px;padding:6px 0;font-size:12px;border-bottom:1px solid var(--border);">
              <span style="display:inline-block;width:6px;height:6px;border-radius:50%;background:${c.pnl >= 0 ? 'var(--red)' : 'var(--green)'};flex-shrink:0;"></span>
              <span style="flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${s.name || '未知'}</span>
              <span style="${pnlColor(c.pnl)}font-weight:600;">${c.pnl >= 0 ? '+' : ''}${fmtWan(c.pnl)}</span>
            </div>`;
          }).join('')}
        </div>
      </div>
    ...
    <!-- 大师投票席最新信号 -->
    <div style="background:var(--bg2);border-radius:12px;padding:16px;border:1px solid var(--border);">
      <div style="font-size:13px;font-weight:700;margin-bottom:12px;">🎭 大师投票席 · 最新市场判断</div>
      <div id="dash-vote-signals" style="display:flex;gap:16px;flex-wrap:wrap;">
        <div style="font-size:12px;color:var(--text3);">请在「大师投票席」页面查看最新信号</div>
      </div>
    </div>
  `;

  // 渲染净值走势图
  setTimeout(() => renderDashNavChart(fundStats, holdingFunds), 100);
  // 渲染资产配置饼图
  setTimeout(() => renderDashPieChart(fundAssets, stockAssets, futuresAssets), 100);
}
...
function renderDashNavChart(stats, holdingFunds) {
  ...
  // 只取第一个持仓基金做演示
  const fund = holdingFunds.length > 0 ? holdingFunds[0] : null;
  const navHistory = fund && fund.navHistory ? fund.navHistory : [];
  // 模拟数据
  const labels = navHistory.length > 0 ? navHistory.map(p => p.date).slice(-30) : ['暂无数据'];
  const data = navHistory.length > 0 ? navHistory.map(p => Number(p.nav).toFixed(4)) : [1];
  ...
  window._dashNavChart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        x: { display: false },
        y: { display: true, grid: { color: 'var(--border)' }, ticks: { font: { size: 9 }, color: 'var(--text3)' } }
      }
    }
  });
}
  ...
  window._dashPieChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: hasData ? ['var(--blue)', 'var(--red)', 'var(--yellow)'] : ['var(--border)', 'var(--border)', 'var(--border)'],
        borderWidth: 0,
      }]
    },
    options: {
...
function openPnlDetail(type) {
  console.log('openPnlDetail called, type:', type);
  console.log('funds:', funds.length, 'items');
  const stats = getPEStats();
  const allFunds = funds;
